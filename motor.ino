...
int delta_x,delta_y,delta_z,delta_r;
int over_x,over_y,over_z,over_r;
int steps_total;
int steps_taken;
...
void motor_set_step_count(long x,long y,long z,long r) {
  wait_for_empty_segment_buffer();

  Segment &old_seg = line_segments[get_prev_segment(last_segment)];
  old_seg.a[0].step_count=x;
  old_seg.a[1].step_count=y;
  old_seg.a[2].step_count=z;
  old_seg.a[3].step_count=r;
...
ISR(TIMER1_COMPA_vect) {
  ...
  if( working_seg == NULL ) {
    ...
    if( working_seg != NULL ) {
      ...
      delta_r = working_seg->a[3].absdelta;
      over_x = -(steps_total>>1);
      over_y = -(steps_total>>1);
      over_z = -(steps_total>>1);
      over_r = -(steps_total>>1);
 ...
 **/
void motor_line(long x,long y,long z,long r,float new_feed_rate) {
...
*/
  new_seg.a[0].step_count = x;
  new_seg.a[1].step_count = y;
  new_seg.a[2].step_count = z;
  new_seg.a[3].step_count = r;
  new_seg.a[0].delta = x - old_seg.a[0].step_count;
  new_seg.a[1].delta = y - old_seg.a[1].step_count;
  new_seg.a[2].delta = z - old_seg.a[2].step_count;
  new_seg.a[3].delta = r - old_seg.a[3].step_count;
  ...
  if(last_segment != current_segment) {
    ...
    float dsr = new_seg.a[3].delta_normalized - old_seg.a[3].delta_normalized;
    float jerk = sqrt(dsx*dsx + dsy*dsy + dsz*dsz + dsr*dsr);
